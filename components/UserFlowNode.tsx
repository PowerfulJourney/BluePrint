...
const UserFlowNode = ({ id, data, selected }: NodeProps<UserFlowNodeData>) => {
  ...
  return (
    <div className="w-[340px] relative group/node">
      <Handle type="target" position={Position.Top} className="!bg-gray-300 !w-3 !h-3 !border-2 !border-white transition-colors hover:!bg-blue-500" />
      
      {/* Main Content Card - Removed overflow-hidden to let popovers escape */}
      <div 
          className={`bg-white rounded-xl shadow-lg border transition-all duration-300 flex flex-col ${selected ? 'border-blue-500 ring-2 ring-blue-100 shadow-xl' : 'border-gray-200 hover:border-gray-300'}`}
      >
        {/* Node Header - Reduced bottom padding to lessen gap */}
        ...
        <div className="px-3 pb-3 space-y-3 flex-1">
          ...
          {/* Render Children - Flex Wrap allows collapsed items to sit next to each other */}
          <div className="flex flex-wrap gap-2 content-start">
               {data.children.map((widget) => {
                  const commonProps = {
                      data: widget as any, // TS gymnastics for union type
                      onDelete: () => removeWidget(id, widget.id),
                      onToggleCollapse: () => toggleWidgetCollapse(id, widget.id)
                  };

                  if (widget.type === WidgetType.OPPORTUNITY) {
                      return <OpportunityRenderer key={widget.id} {...commonProps} onUpdate={(d) => updateWidget(id, widget.id, d)} />;
                  }
                  if (widget.type === WidgetType.DECISION) {
                      return <DecisionRenderer key={widget.id} {...commonProps} onUpdate={(d) => updateWidget(id, widget.id, d)} />;
                  }
                  if (widget.type === WidgetType.TODO) {
                      return <TodoRenderer key={widget.id} {...commonProps} onUpdate={(d) => updateWidget(id, widget.id, d)} />;
                  }
                  return null;
              })}
          </div>
        ...
        {/* Footer Actions - Redesigned: Horizontal, Darker, No Top Border. Added rounded-b-xl to match container since overflow is visible now. */}
        <div className="px-2 py-2 bg-gray-50/50 flex justify-between items-center gap-1 rounded-b-xl">
          <button 
              onClick={() => addInteractionWidget(id, WidgetType.OPPORTUNITY)}
              className="group flex-1 py-1.5 flex flex-row items-center justify-center gap-1.5 rounded-md hover:bg-yellow-50/50 transition-all"
              title="添加机会点"
          >
      ...
      <Handle type="source" position={Position.Bottom} className="!bg-gray-300 !w-3 !h-3 !border-2 !border-white transition-colors hover:!bg-blue-500" />
    </div>
  );
};
